...
const FileIcon = ({ file, width = "36px", height = "36px", fileName }) => {
  ...
  useEffect(() => {
    switch (fileExtension) {
      ...
      case "jpeg":
      case "gif":
        setIcon(import.meta.env.VITE_BASEURL + file);
        break;
      case "docx":
      case "doc":
        setIcon(docIcon);
  ...
  return (
    <div className="flex space-x-2 pe-4 items-center ">
      ...
      />
      <span className="text-lg">{truncateString(fileName(file), 15)}</span>
    </div>
  );
};
